...
至此，完整的认证功能测试完成。建议在`SysUserServiceImpl`类中，从`authenticationManager.authenticate()`方法开始设置断点，结合 [Spring Security 登录流程](/code/java/authentication.html#_2-2-spring-security-登录流程) 进行调试，以便更深入地理解整个认证流程的执行过程。
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .exceptionHandling(exception -> exception.authenticationEntryPoint(authenticationEntryPoint))
                // 登出配置
                .logout(logout -> logout.logoutUrl("/logout").logoutSuccessHandler(logoutSuccessHandler)) // [!code ++]
                .build();
    }
}
```
