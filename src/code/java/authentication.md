...
::: tip
Spring Security 登录流程涉及多个组件和步骤，对应的时序图较为复杂且篇幅较大，建议将其下载后放大查看以获得更好的阅读体验。此外，需要设置断点的关键代码位置均已添加高亮标记，请仔细查看相关代码块。

Spring Security 的登录认证流程虽然涉及多个环节，但整体逻辑清晰，通过仔细阅读文档和时序图，并结合实际调试，可以很好地理解整个认证过程。文档下方已详细列出各个关键步骤和代码位置，便于开发者进行学习和调试。

最后，作者本人也是初学者，整个流程是参考[网络文档](/code/java/authentication.html#_10-参考)并结合代码调试梳理而成，如有错误欢迎在git上评论或修改。
:::
...
3. `AuthenticationManager`接收`UsernamePasswordAuthenticationToken`，其具体实现类`ProviderManager`调用`authenticate()`方法进行认证处理。
   ...
   public Result login(LoginDTO loginDTO) {
       ...
       try {
           authenticate = authenticationManager.authenticate(usernamePasswordAuthenticationToken); // [!code highlight]
...
4. `ProviderManager`管理多个`AuthenticationProvider`实现，遍历所有`AuthenticationProvider`找到能处理当前`authentication`的`provider`。
   ...
   for (AuthenticationProvider provider : getProviders()) {
       if (provider.supports(authentication.getClass())) {
           Authentication result = provider.authenticate(authentication); // [!code highlight]
           if (result != null) {
               return result;
           }
       }
...
5. `ProviderManager`找到`AbstractUserDetailsAuthenticationProvider`，该 provider 调用`authentication`方法，内部再调用抽象方法`retrieveUser()`，由其子类`DaoAuthenticationProvider`负责从数据库检索用户信息。
   ...
   if (user == null) {
       cacheWasUsed = false;
       try {
           user = retrieveUser(username, (UsernamePasswordAuthenticationToken) authentication); // [!code highlight]
...
6. `DaoAuthenticationProvider`调用`retrieveUser()`，获取配置的`UserDetailsService`，然后调用自定义实现的`loadUserByUsername()`方法查询数据库用户信息。
   ...
   try {
       UserDetails loadedUser = this.getUserDetailsService().loadUserByUsername(username); // [!code highlight]
       if (loadedUser == null) {
...
8. 将用户信息封装到`LoginUser`后返回到`AbstractUserDetailsAuthenticationProvider`的`authentication`方法，继续执行后续验证流程。
   ...
   ```java title="AbstractUserDetailsAuthenticationProvider.authenticate()关键代码"
   try {
   		this.preAuthenticationChecks.check(user); // [!code highlight]
...
9. `this.preAuthenticationChecks.check(user)`内部会校验 [登录实体类](/code/java/authentication.html#_5-2-登录实体类) 中实现的`UserDetails`接口方法返回值，如检查账户是否被锁定、是否过期等状态。
10. 抽象方法`additionalAuthenticationChecks()`会调用`DaoAuthenticationProvider`的具体实现。首先判断 [登录实体类](/code/java/authentication.html#_5-2-登录实体类) 中重写的`getPassword()`方法是否为空，然后调用`matches()`方法，按照 [Spring Security 配置](/code/java/authentication.html#_3-spring-security-配置) 中配置的密码加密方式进行密码匹配。
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails,
        ...
        String presentedPassword = authentication.getCredentials().toString(); // [!code highlight]
...
11. 本项目配置的是`BCryptPasswordEncoder`，进入`matches()`方法后，先进行格式校验，验证密码是否符合 BCrypt 格式，之后进入`checkpw()`方法进行密码比对。
    ...
    public boolean matches(CharSequence rawPassword, String encodedPassword) {
    	...
    	return BCrypt.checkpw(rawPassword.toString(), encodedPassword); // [!code highlight]
    }
    ```
...
12. `checkpw()`方法接收两个参数：用户输入的未加密密码和数据库中存储的加密密码。将未加密密码转换为字节数组后传入`hashpwforcheck()`方法进行加密处理，最后通过`MessageDigest.isEqual()`方法安全地比较两个密码。
    ...
    public static boolean checkpw(String plaintext, String hashed) {
    	...
    	return equalsNoEarlyReturn(hashed, hashpwforcheck(passwordb, hashed)); // [!code highlight]
    }
...
14. 登录成功后，后续请求中前端会将 Token 存入请求头中。在`JwtAuthenticationTokenFilter`中获取用户信息，并将用户存入`SecurityContextHolder`，方便在业务代码中获取当前用户信息。
...
::: important
1. 默认情况下，FastJSON2 会通过`getter`方法来序列化对象属性，对于重写的字段需要添加`@JSONField(serialize = false)`注解以取消序列化这些不必要的属性。也可以在[Redis 序列化器](/code/java/redis.html#_2-2-redis-序列化配置)中配置`JSONWriter.Feature.FieldBased`特性，基于字段而非`getter`方法进行序列化，提高性能和一致性。
2. 在使用 FastJSON 反序列化 Redis 中的对象时，由于 DevTools 使用了不同的类加载器（RestartClassLoader）加载应用类，而 FastJSON 使用默认类加载器，这可能导致反序列化后得到的是`JSONObject`而非期望的对象类型。在[Redis 序列化器](/code/java/redis.html#_2-2-redis-序列化配置)中已配置`JSONWriter.Feature.WriteClassName`特性，通过写入类名信息确保反序列化时能正确还原对象类型。此外，也可以从 Redis 中查询到数据后，先通过`toString()`方法转换为字符串，再使用`JSONObject.parseObject()`来避免反序列化失败的问题。
:::
...
如果上一步过滤器中没有获取到用户，在 `doFilter` 的过程中会进入此处理器，向前端返回 401，同样需要在 SecurityConfig 中进行配置。
...
完成上述认证授权功能的开发后，可通过前端页面进行功能验证。请将页面文件放置在 `/resources/statics` 目录下。

前端页面代码及完整验证流程详见 [认证功能测试](/code/java/authentication-test.html)。

::: tip 关键验证步骤
1. 访问 `http://localhost:8888/test-db.html`，使用生成的测试密码登录
2. 登录后在 Redis 中查看缓存的用户数据
3. 点击按钮验证 Token 传递和接口认证
4. 清除 Token 后验证拦截跳转
5. 修改 Redis 中过期时间至 30 分钟内，验证 Token 刷新

建议在 `SysUserServiceImpl` 中从 `authenticationManager.authenticate()` 开始设置断点，结合 [Spring Security 登录流程](#_2-2-spring-security-登录流程) 进行调试。
:::

## 10.退出登录

退出登录功能的实现相对简单。只需实现`LogoutSuccessHandler`接口，在`onLogoutSuccess`方法中清除 Redis 中的用户缓存，并将成功的响应结果返回给前端。最后，还需在`SecurityConfig`类中进行相应的配置。

### 10.1 TokenService

在`TokenService`类中新增`deleteLoginUser`方法，用于删除 Redis 缓存中的用户信息。
...
### 10.3 配置 SecurityConfig

在 [3.1 安全配置类](#_3-1-安全配置类) 的 `filterChain()` 方法中补充登出配置：

```java
// SecurityConfig.filterChain() 中新增：
...
前端测试页面的退出登录功能已包含在 [认证功能测试](/code/java/authentication-test.html) 中。
