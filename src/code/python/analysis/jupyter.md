...
```ipynb title = "01.Jupyter演示.ipynb"
a = 100
b = 200
c = 300
a + b + c
...
![添加正在运行本地服务器](/assets/image/code/python/analysis/jupyter/addLocalServer.png)

4. 安装完成后会提示 `没有正在运行的本地服务器`。关闭配置页面，在命令行执行 `jupyter lab` 命令，系统会自动运行并打开浏览器。

![运行 Jupyter](/assets/image/code/python/analysis/jupyter/runJupyter.png)
...
![在浏览器打开 Notebook](/assets/image/code/python/analysis/jupyter/openOnBrowser.png)
