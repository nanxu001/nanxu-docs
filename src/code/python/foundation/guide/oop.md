...
**面向过程编程**：将一个大的需求拆解为一系列要执行的步骤，然后按照步骤依次执行。

**面向对象编程**：一种编程思想，通过类和对象来组织代码，将数据和操作数据的方法封装在一起。

::: info 说明
面向对象编程更易于维护和扩展，适合大型项目开发。
:::
...
::: info 说明
魔法方法是 Python 提供的以双下划线开头和结尾的特殊方法，用于定义类的特殊行为。魔法方法不需要手动调用，Python 解释器会在合适的时间自动调用。
:::
...
在完成面向对象案例时，首先需要根据需求描述来提炼梳理，考虑：
1. 涉及哪些**业务实体**（具体的事物或参与者）
2. 每个实体有哪些**属性**和**方法**

**本案例涉及的实体：**
- **教务管理系统**：管理学生成绩信息，提供增删改查功能
- **学生**：包含姓名、语文成绩、数学成绩、英语成绩
