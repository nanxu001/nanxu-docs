...
| `\d` | 匹配数字 `[0-9]` | `\d+` 匹配 "123" |
| `\D` | 匹配非数字 | `\D+` 匹配 "abc" |
| `\s` | 匹配空白字符（空格、换行、制表符等） | `\s` 匹配 " " |
| `\S` | 匹配非空白字符 | `\S+` 匹配 "abc" |
| `\w` | 匹配单词字符（字母、数字、下划线，**含中文**） | `\w+` 匹配 "hello_123"、"你好" |
| `\W` | 匹配非单词字符（不含中文） | `\W` 匹配 "!"、"@" |
...
# \s 匹配空白字符
s2 = "hello world 你好"
print(re.findall(r"\s", s2))     # [' ', ' ']

# \S 匹配非空白字符
print(re.findall(r"\S+", s2))    # ['hello', 'world', '你好']

# \w 匹配单词字符（Python3 默认按 Unicode 匹配，包含中文）
print(re.findall(r"\w+", s2))    # ['hello', 'world', '你好']
```
...
分组除了**捕获**内容，还能在后续**引用**之前匹配到的分组——常用于校验前后文本是否一致（如 HTML 标签闭合）：

| 语法 | 说明 | 示例 |
|:---:|:---:|:---:|
| `\num` | 引用第 num 组匹配的内容 | `<(\w+)>.*</\1>` 匹配 `<html>...</html>` |
| `(?P<分组名>)` | 给分组起名 | `(?P<tag>h1)` |
| `(?P=分组名)` | 引用命名分组 | `</(?P=tag)>` |

```python title="10.正则表达式-分组引用.py"
import re

# 需求1：校验单级标签，如 <html>我是html页面</html>
html_s = "<html>我是html页面</html>"

# 写法1：前后各写一遍，标签不一致也能通过（如 </div> 也能闭合 <html>）
result = re.match(r"^<[a-zA-Z]{1,4}>.*</[a-zA-Z]{1,4}>$", html_s)

# 写法2：用 \1 引用第1组，强制前后标签一致
result = re.match(r"^<([a-zA-Z]{1,4})>.*</\1>$", html_s)

if result:
    print(result.group())  # <html>我是html页面</html>
else:
    print("未匹配")


# 需求2：校验嵌套标签，如 <html><h1>我是html页面</h1></html>
html_s = "<html><h1>我是html页面</h1></html>"

# 写法1：重新copy一份
result = re.match(r"^<[a-zA-Z]{1,4}><[a-zA-Z]{1,4}>.*</[a-zA-Z]{1,4}></[a-zA-Z]{1,4}>$", html_s)

# 写法2：用 \num 按编号引用分组
result = re.match(r"^<([a-zA-Z]{1,4})><(h[1-6])>.*</\2></\1>$", html_s)

# 写法3：给分组起名，用 (?P=分组名) 引用
result = re.match(r"^<(?P<A>[a-zA-Z]{1,4})><(?P<B>h[1-6])>.*</(?P=B)></(?P=A)>$", html_s)

if result:
    print(result.group())  # <html><h1>我是html页面</h1></html>
else:
    print("未匹配")
```
...
除了提取和验证，正则还可以做**替换**——把匹配到的内容替换成指定字符串。`re.sub()` 和 `re.compile().sub()` 都能实现：

```python title="09.正则表达式-替换.py"
import re

s = "开心你就大声笑，哈哈，呵呵，嘿嘿，嘻嘻，桀桀桀，啦啦啦"

# re.sub(正则, 替换内容, 字符串)：一次性替换，写法最简洁
result = re.sub("哈|呵|嘿|嘻|桀", "♥", s)
print(result)  # 开心你就大声笑，♥♥，♥♥，♥♥，♥♥，♥♥♥，啦啦啦

# 等价写法：先 compile 编译再替换，同一个正则需要重复使用时更高效
result2 = re.compile("哈|呵|嘿|嘻|桀").sub("♥", s)
print(result2)  # 开心你就大声笑，♥♥，♥♥，♥♥，♥♥，♥♥♥，啦啦啦
```

::: tip 两种写法的区别
- `re.sub()`：一次性替换，写法最简洁
- `re.compile().sub()`：先把正则**编译**成模式对象，同一个正则要**重复使用**（多次匹配、替换）时推荐——只编译一次，避免重复解析
:::
